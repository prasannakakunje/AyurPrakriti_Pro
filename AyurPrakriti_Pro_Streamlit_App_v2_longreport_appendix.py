...
# Added: Option B (Appendix) for long-form recommendations in PDF (toggle in UI)
...
# ---------------- LONG-FORM APPENDIX TEXT ----------------------------------
# Long-form sections to be appended to PDF when include_appendix=True
LONG_APPENDIX_TEXT = {
    "executive_addendum": """This addendum expands the clinical interpretation: a Vata-dominant constitution means the patient benefits from routines, warmth, and nourishment that ground mobility (both physical and mental). The current mild–moderate Kapha uplift suggests transient heaviness, congestion, or inertia — possibly due to seasonal factors, recent dietary choices, or sleep patterns. Therapeutic priorities: (1) stabilise Vata through predictable routine, nourishing oils and gentle warming foods; (2) reduce Kapha stagnation with light, stimulating activity and drying, thermogenic spices; (3) design career and lifestyle choices that convert Vata’s creativity into sustainable work (short projects, flexible hours, roles that allow novelty but with predictable structure). The following recommendations are prioritised by immediacy (what to start in 24–72 hours), short term (2–6 weeks), and medium term (2–6 months).""",
    "career_long": """Vata personalities excel in ideation, pattern-recognition, conceptual work and quick learning. Best roles harness creative output with intermittent deep work windows: copywriting, creative design, UX research, product ideation, scientific writing, or independent consulting. Avoid roles that demand long, repetitive procedural tasks with heavy admin unless they include structures and automation.

90-day plan (practical):
- Week 0–2: Pick 2 micro-projects (one creative writing piece, one short research summary). Time-box work into 90-minute deep-focus windows and create a simple Kanban board.
- Week 3–8: Publish the micro-projects, seek micro-freelance opportunities and collect feedback.
- Week 9–12: Convert the successful tasks into a repeatable service and introduce a fixed weekly schedule to reduce decision fatigue.

Skills to invest: professional writing, UX fundamentals, basic data literacy. Tools for structure: Notion/Trello and simple proposal templates. Monitor weekly deep-work hours and monthly client feedback to adapt workload.""",
    "relationship_long": """Objective: Reduce interpersonal unpredictability (which increases Vata anxiety) and increase perceived reliability by partner/colleagues through small consistent behaviours.

Daily micro-rituals:
- Shared 10-minute check-in at a fixed time to exchange top items and one appreciation.
- Predictable communication windows to reduce surprise and overload.

Communication scripts:
- When overwhelmed: “I’m feeling scattered and need five minutes to collect my thoughts; can we speak at 8:40?”
- To de-escalate: “I hear you — let me repeat what I understood. Is this accurate?”

Structure: Use a shared weekly plan with clear micro-tasks (grocery, bills, family time). If miscommunication persists after 3+ weeks, consider a guided couples session or structured conflict coaching. Include short breathing practice before difficult conversations if emotional reactivity is high.""",
    "health_long": """Primary goal (first 2–7 days): Ground Vata with warmth, routine, oil, and sleep hygiene.
Secondary goal (2–6 weeks): Reduce Kapha stagnation via stimulation, thermogenic spices, light exercise, and digestive clarity.
Monitoring: Sleep quality, stool pattern, morning energy score, and weight trend.

Diet (sample daily):
- Morning: Warm lemon water; breakfast: warm kitchari with a teaspoon of ghee or cooked oats with warm milk and spices.
- Lunch: Warm cooked vegetables with whole grains and a light dal; use hing and trikatu spices for digestion.
- Evening: Warm spiced tea and a light dinner by 8 PM.

Lifestyle (Dinacharya):
- Abhyanga (warm oil self-massage) 5–10 minutes before shower daily for 7–21 days, then 3–4x/week.
- Wake time consistent (6–6:30 AM); sleep by 10 PM; 10 minutes of breathwork morning and evening; 20–30 minutes grounding yoga daily; brisk walk 30 minutes 5x/week for Kapha.

Herbs & safety:
- Ashwagandha (start low, e.g., 250 mg at night) for sleep/resilience; Bala for strength; Trikatu for digestion. Avoid herbal self-prescription if on medications or pregnancy; consult clinician.

Clinic referrals & follow-up:
- 2-week review: phone/brief clinic review (sleep, stool, energy).
- 6-week review: progress check and objective measures if needed (BP, blood tests).
Red flags: palpitations, high fever, persistent diarrhoea, severe insomnia (>7 nights), or severe mood symptoms requiring urgent care.""",
    "patient_handout": """Patient one-page handout — Essentials (first 2 weeks):
1. Wake & sleep at similar times (wake ~6–6:30 AM; sleep by 10 PM).
2. Daily oil massage (Abhyanga) 5–10 min before shower (warm sesame oil).
3. Eat warm cooked meals at regular times; avoid iced drinks and raw salads for first 2 weeks.
4. Walk briskly 30 minutes daily and do 10 minutes grounding breathing twice daily.
5. Start Ashwagandha low dose only after clinician discussion.

Contact clinic for: palpitations, persistent high fever, severe insomnia, sudden weight loss, or severe emotional distress.""",
}


# ----------------- PDF/APPENDIX HELPERS ------------------------------------
...
def _fallback_canvas_pdf(
    ...
    """
    Readable canvas fallback (BytesIO). Includes error snippet and optional appendix.
    """
    buf = BytesIO()
    ...
    # Include appendix text if requested
    if include_appendix:
        c.showPage()
        y = top
        c.setFont("Helvetica-Bold", 12)
        c.drawString(left, y, "APPENDIX — Detailed Ayurvedic Recommendations")
        y -= 16
        c.setFont("Helvetica", 9)
        for block in [
            "executive_addendum",
            ...
            for line in _wrap_text_simple(LONG_APPENDIX_TEXT[block], 95):
                c.drawString(left, y, line)
                y -= 10
                if y < 50 * mm:
                    c.showPage()
                    y = top
            y -= 6
        # patient handout on separate page
        c.showPage()
        y = top
        c.setFont("Helvetica-Bold", 11)
        c.drawString(left, y, "Patient Handout — Essentials (first 2 weeks)")
        y -= 14
        c.setFont("Helvetica", 9)
        for line in _wrap_text_simple(LONG_APPENDIX_TEXT["patient_handout"], 95):
            c.drawString(left, y, line)
            y -= 10
            if y < 30 * mm:
                c.showPage()
...
def branded_pdf_report(
    ...
    include_appendix=True -> append detailed long-form appendix pages (Option B)
    """
    logger.info(
    ...
    try:
        ...
        # If include_appendix is True, add Appendix pages (long-form)
        if include_appendix:
            flow.append(PageBreak())
            flow.append(
                Paragraph(
                    "APPENDIX — Detailed Ayurvedic Recommendations",
                    styles["AP_Heading"],
                )
            )
            flow.append(Spacer(1, 6))
            # executive addendum
            flow.append(Paragraph("<b>Executive Addendum</b>", styles["AP_Body"]))
            flow.append(
                Paragraph(LONG_APPENDIX_TEXT["executive_addendum"], styles["AP_Body"])
            )
            flow.append(Spacer(1, 6))
            # career long
            flow.append(Paragraph("<b>Career — Expanded Plan</b>", styles["AP_Body"]))
            for line in LONG_APPENDIX_TEXT["career_long"].split("\n"):
                flow.append(Paragraph(line.strip(), styles["AP_Body"]))
            flow.append(Spacer(1, 6))
            # relationship long
            flow.append(
                Paragraph(
                    "<b>Relationship & Communication — Expanded</b>", styles["AP_Body"]
                )
            )
            for line in LONG_APPENDIX_TEXT["relationship_long"].split("\n"):
                flow.append(Paragraph(line.strip(), styles["AP_Body"]))
            flow.append(Spacer(1, 6))
            # health long
            flow.append(
                Paragraph(
                    "<b>Health — Diet, Lifestyle, Herbs & Monitoring</b>",
                    styles["AP_Body"],
                )
            )
            for line in LONG_APPENDIX_TEXT["health_long"].split("\n"):
                flow.append(Paragraph(line.strip(), styles["AP_Body"]))
            flow.append(PageBreak())
            # patient handout
            flow.append(
                Paragraph(
                    "Patient Handout — Essentials (First 2 weeks)", styles["AP_Heading"]
                )
            )
            for line in LONG_APPENDIX_TEXT["patient_handout"].split("\n"):
                flow.append(Paragraph(line.strip(), styles["AP_Body"]))
...
# ---------------- Tab 2: New Assessment (REPLACEMENT) -----------------------
with tabs[1]:
    ...
    if patients.empty:
        ...
        # Show long preview checkbox (preview uses session or freshly computed)
        show_long_preview = st.checkbox(
            "Show long recommendations on screen (preview)", value=False
        )

        # Compute & Save button -> computes and stores results to session_state
        if st.button("Compute & Save Assessment"):
            ...
            # SAVE into DB and session_state so results persist across reruns
        ...
        # ----------------- DISPLAY results & download controls (persistent) -----------------
        if "last_assessment" in st.session_state:
            ...
            st.markdown("---")
            # IMPORTANT: include_appendix checkbox is outside compute block and uses stored payload
            ...
            # Build PDF on demand using stored payload (so toggling does not remove buttons)
            if st.button("Prepare Branded PDF (ready to download)"):
                ...
                # Save pdf into session so repeated toggles of the checkbox can simply regenerate if needed
                st.session_state["last_pdf"] = pdf_b.getvalue()
                st.success("PDF prepared — use the download button below.")

            # If pdf has been prepared in this session show download button
            if "last_pdf" in st.session_state:
                ...
                # allow direct on-the-fly download by generating when user hits the download button (slower)
                pdf_b = branded_pdf_report(
                    payload["patient"],
                    prak_pct,
                    vik_pct,
            ...
            # Show long preview if requested (uses session payload so stable)
            if show_long_preview:
                st.markdown("---")
                st.header("Detailed Appendix (preview)")
                st.subheader("Executive addendum")
                st.write(LONG_APPENDIX_TEXT["executive_addendum"])
                st.subheader("Career — Expanded plan")
                st.write(LONG_APPENDIX_TEXT["career_long"])
                st.subheader("Relationship & Communication — Expanded")
                st.write(LONG_APPENDIX_TEXT["relationship_long"])
                st.subheader("Health — Diet, Lifestyle, Herbs & Monitoring")
                st.write(LONG_APPENDIX_TEXT["health_long"])
                st.subheader("Patient handout — Essentials (first 2 weeks)")
                st.write(LONG_APPENDIX_TEXT["patient_handout"])

        else:
            st.info(
                'Compute an assessment to see results and download options. After computing, you can toggle the "Include detailed Appendix" checkbox without losing the download buttons.'
            )
